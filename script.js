...
/* =========================================================================
   ...
   /* =========================================================================
      11.B — HERO PREMIUM (tema vermelho)
      ========================================================================= */
   
   function initHeroPremium(){
     const wrap = $('#heroFeaturedCard');
     if (!wrap) return;
   
     /* Contador da coleção em tempo real */
     const colEl = $('#heroColCount');
     if (colEl){
       colEl.textContent = Favorites.all().length;
       if (!colEl.dataset.bound){
         colEl.dataset.bound = '1';
     ...
     /* Busca rápida do hero → manda para a Pokédex */
     const search = $('#heroSearch');
     if (search && !search.dataset.done){
       search.dataset.done = '1';
     ...
     /* Card de destaque — carrega uma vez */
     if (wrap.dataset.loaded) return;
     wrap.dataset.loaded = '1';
   
     wrap.innerHTML = '';
     wrap.appendChild(el('div', { class: 'skeleton skeleton--card', 'aria-hidden': 'true' }));
   
     const pool = [150, 25, 6, 448, 384, 133, 149, 94, 658, 887];
     const id = pool[Math.floor(Math.random() * pool.length)];
   
     PokeAPI.getPokemon(id)
       .then(function(pokemon){
         wrap.innerHTML = '';
         wrap.appendChild(buildHeroFeaturedCard(pokemon));
       })
       .catch(function(){
         wrap.innerHTML = '';
   ...
   function buildHeroFeaturedCard(pokemon){
     const typeKeys = (pokemon.types || []).map(function(t){ return t.type.name; });
     const primaryKey = typeKeys[0] || 'normal';
     const primaryMeta = TYPES[primaryKey] || TYPES.normal;
     const name = prettyName(pokemon.name);
     const isFav = Favorites.has(pokemon.id);
     ...
     const favBtn = el('button', {
       class: 'hero-premium__card-btn hero-premium__card-btn--ghost' + (isFav ? ' is-fav' : ''),
       type: 'button',
       'aria-pressed': isFav ? 'true' : 'false'
     }, [document.createTextNode(isFav ? '♥ Favoritado' : '♡ Favoritar')]);
   
     favBtn.addEventListener('click', function(){
       const list = Favorites.toggle(pokemon.id);
       const nowFav = list.indexOf(Number(pokemon.id)) !== -1;
       favBtn.textContent = nowFav ? '♥ Favoritado' : '♡ Favoritar';
     ...
     const viewBtn = el('button', {
       class: 'hero-premium__card-btn hero-premium__card-btn--primary',
       type: 'button',
       onclick: function(){ location.hash = '#pokemon?id=' + pokemon.id; }
     }, [document.createTextNode('Ver carta')]);
   
     const typeBadgeEl = el('span', { class: 'hero-premium__card-type' }, [
       el('span', { 'aria-hidden': 'true', text: primaryMeta.icon }),
       document.createTextNode(primaryMeta.pt.toUpperCase())
     ]);
   
     return el('div', { class: 'hero-premium__card-inner', style: 'position:relative;z-index:1' }, [
       el('div', { class: 'hero-premium__card-top' }, [
         el('span', { class: 'hero-premium__card-num', text: '# ' + pokemon.id }),
         el('span', { class: 'hero-premium__card-tag', text: '★ DESTAQUE' })
       ]),
       el('div', { class: 'hero-premium__card-art' }, [img]),
       el('h3', { class: 'hero-premium__card-name', text: name }),
       el('div', { class: 'hero-premium__card-types' }, [typeBadgeEl]),
       el('div', { class: 'hero-premium__card-actions' }, [viewBtn, favBtn]),
       el('div', { class: 'hero-premium__card-dots' }, [
         el('span', { class: 'hero-premium__dot' }),
         el('span', { class: 'hero-premium__dot' }),
         el('span', { class: 'hero-premium__dot is-active' })
       ])
     ]);
   }
